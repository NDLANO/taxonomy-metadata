...
public interface CustomFieldValueRepository extends JpaRepository<CustomFieldValue, UUID> {
    @Query(
            "SELECT obj FROM CustomFieldValue obj WHERE obj.taxonomyEntity.id = :taxonomyEntity AND obj.customField.id = :customField")
    Optional<CustomFieldValue> findByTaxonomyEntityAndCustomField(
            UUID taxonomyEntity, UUID customField);
    ...
    @Query(
            "SELECT obj FROM CustomFieldValue obj LEFT JOIN FETCH obj.taxonomyEntity WHERE obj.customField.id = :customField AND obj.value = :value")
    Iterable<CustomFieldValue> findAllByCustomFieldAndValue(UUID customField, String value);

    @Query(
            "SELECT obj FROM CustomFieldValue obj LEFT JOIN FETCH obj.taxonomyEntity WHERE obj.customField.id = :customField")
